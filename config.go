...
func (c *CmdLineConfig) Init(args []string) {
	...
	argServerPrometheusRuntimeMetrics := flags.Bool("prometheus-runtime-metrics", false, "Include Go runtime and process metrics")
	argServerOTelEnabled := flags.Bool("otel-enabled", false, "Enable OpenTelemetry export")
	argServerOTelTracesEnabled := flags.Bool("otel-traces-enabled", false, "Export OpenTelemetry traces when OTel is enabled")
	...
	argServerOTLPInsecure := flags.Bool("otel-exporter-otlp-insecure", false, "Use insecure OTLP HTTP transport")
